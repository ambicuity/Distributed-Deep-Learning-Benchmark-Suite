...
class ReportGenerator:
    ...
    def calculate_scaling_efficiency(self, results: List[Dict]) -> pd.DataFrame:
        ...
        if 'avg_throughput' in df.columns and 'gpu_count' in df.columns:
            ...
            df['ideal_throughput'] = df.groupby(['model', 'batch_size'])['avg_throughput'].transform(
                lambda x: x.iloc[0] * df.loc[x.index, 'gpu_count'] / df.loc[x.index, 'gpu_count'].iloc[0]
            )
            df['scaling_efficiency'] = (df['avg_throughput'] / df['ideal_throughput']) * 100
    ...
    def _create_html_content(self, df: pd.DataFrame, profiling_results: List[Dict]) -> str:
...
"""
        ...
        if not df.empty:
            ...
            html += df[['model', 'batch_size', 'gpu_count', 'avg_throughput', 'iteration_latency', 'scaling_efficiency']].to_html(
                index=False,
            ...
            for result in profiling_results:
                ...
                if bottlenecks:
                    ...
                    for bottleneck in bottlenecks:
                        ...
                        html += f'<strong>{bottleneck.get("type", "Unknown")}</strong><br>'
